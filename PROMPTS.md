...
## 2026-09-22 — Résolution des anomalies post-merge (CORS/HTTPS et bandeau d'erreur Blazor)

**Outil / modèle** : Antigravity (Gemini)

**Contexte** : après la fusion des branches sur `main`, deux problèmes bloquaient l'utilisation fluide du jeu : l'API forçait une redirection HTTPS introuvable en profil de dev `http` bloquant les appels du front, et le bandeau d'erreur Blazor `#blazor-error-ui` s'affichait en permanence en bas de page dès le chargement initial.

**Prompt** : « L'API refuse les requêtes avec une erreur de redirection HTTPS, et dans le navigateur j'ai une bannière "An unhandled error has occurred. [Reload]" affichée en bas de page alors que le jeu tourne. Analyse les causes réelles de ces deux problèmes et corrige-les proprement. »

**Réponse résumée** :
1. Suppression de `UseHttpsRedirection()` en profil local et ajout de l'origine Blazor (`http://localhost:5270`) dans la politique CORS de `BattleShip.API/Program.cs`.
2. Ajout de la règle manquante `#blazor-error-ui { display: none; ... }` dans `BattleShip.App/wwwroot/css/app.css` (en HTML standard un `div` est en `display: block` par défaut si aucune règle CSS ne le masque).

**Décision** : acceptée. Les deux corrections résolvent immédiatement les régressions et nettoient le rendu dans le navigateur.

**Vérification** : `dotnet test` → 56/56 tests verts, validation de partie complète en direct dans le navigateur sans blocage CORS ni bandeau d'erreur visible.

**Preuve** : commits `ca3a7a0`, `BattleShip.API/Program.cs`, `BattleShip.App/wwwroot/css/app.css`.
